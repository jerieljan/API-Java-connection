...
/**
 * ************************************************************* Send MMS API class template using
 * Apache's HttpClient v4.3.3 Java Version 6. Required libraries/jars: httpcore-4.3.2.jar (and
 * dependencies); httpclient-4.3.3.jar (and dependencies); log4j-1.2.17.jar.
 *
 * NOTE: To use this API, you should already have a token for Authorization.
 *
 * @author MARebultan *************************************************************
 */
...
public class SendMMS {
    ...
    public static void main(String[] args) throws IOException {
        ...
        String apiURL = "https://x.smart.com.ph/1/messaging/outbound/" + senderAddress
                + "/requests";
        String jsonRequest = "{\"outboundMessageRequest\":{\"address\":\"" + address
                + "\",\"senderAddress\":\"" + senderAddress
                + "\",\"outboundMMSMessage\":{\"subject\":\"" + subject + "\"}}}";

        String authorization = "Bearer "
                + token; // This will be the value of your authorization header
        ...
        reqEntityBuilder
                .addBinaryBody("root-fields", new ByteArrayInputStream(jsonRequest.getBytes()),
                        ContentType.create("application/json", Charset.forName("UTF-8")), null);
        reqEntityBuilder.addBinaryBody("Attachments", new FileInputStream(attachmentJPG),
                ContentType.create("image/jpg"), attachmentJPG.getName());
